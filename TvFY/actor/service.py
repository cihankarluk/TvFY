...
class ActorService:
    ...
    @classmethod
    def create_actor(cls, actor_data: dict[str, str]) -> Actor:
    ...
    @classmethod
    def create_multiple_actor(cls, cast_data: List[dict[str, str]]) -> dict[str, dict[str, ...]]:
        """
        ...
        for imdb_url, cast_data in cast_dict.items():
            if actor := existing_actors.filter(imdb_url=imdb_url):
                cast_data["actor"] = actor.get()
            else:
                cast_data["actor"] = cls.create_actor(cast_data)
        return cast_dict

    @classmethod
    def update_actor(cls, actor_obj: Actor, actor_data: dict[str, ...]):
