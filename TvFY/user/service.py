...
class UserService:
    ...
    def get_movies(cls, user: Account):
        query = (
            UserMovies.objects.select_related("movie")
            .prefetch_related(
                "movie__genres",
                "movie__language",
                "movie__country",
        ...
        if user_movies := query:
            ...
            results = {
                ...
                "languages": sorted(
                    ({"country": language, "time_spent": time_spent} for language, time_spent in languages.items()),
        ...
        if series := Series.objects.prefetch_related("season_set").filter(tvfy_code=request_data["tvfy_code"]):
            ...
            if request_data["watched_past_seasons"]:
                ...
                for season in range(1, int(request_data["watched_season"]) + 1):
                    ...
                    user_series = UserSeries.objects.filter(user=user, series=series, watched_season=watched_season)
                    if user_series.exists():
                        results.append(
                            cls.update_user_series(
    ...
    def get_user_series(cls, user: Account) -> list[dict[str, Any]]:
        query = (
            UserSeries.objects.select_related(
                "series",
                "watched_season",
            )
            .prefetch_related(
        ...
        for series, watched_season_list in grouped_series.items():
            last_watched_episode = (
                watched_user_series_query.filter(series=series)
                .order_by("-watched_season__season")
                .first()
                .last_watched_episode
            )
            result.append(
